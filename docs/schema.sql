...
-- Índices para espacios deportivos
...
-- NOTA IMPORTANTE: auth.uid() normalmente retorna UUID. Dado que hemos cambiado a IDs enteros (SERIAL)
-- necesitamos la conversión auth.uid()::TEXT::INTEGER para la compatibilidad.
-- 
-- Alternativamente, se podría crear una tabla de identificadores:
-- CREATE TABLE auth_identifiers (
--   user_id INTEGER PRIMARY KEY REFERENCES usuarios(id),
--   auth_id UUID UNIQUE NOT NULL
-- );
--
-- Y luego modificar las políticas para usar:
-- ... WHERE user_id = (SELECT user_id FROM auth_identifiers WHERE auth_id = auth.uid())
...
CREATE POLICY admin_select_all ON usuarios
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM usuarios WHERE id = auth.uid()::TEXT::INTEGER AND role = 'admin')
  );
...
CREATE POLICY espacios_crud_own ON espacios_deportivos
  USING (propietario_id = auth.uid()::TEXT::INTEGER);
...
CREATE POLICY reservas_select_own ON reservas
  FOR SELECT USING (usuario_id = auth.uid()::TEXT::INTEGER);

-- Los propietarios pueden ver reservas de sus espacios
CREATE POLICY reservas_select_owner ON reservas
  FOR SELECT USING (
    espacio_id IN (
      SELECT id FROM espacios_deportivos WHERE propietario_id = auth.uid()::TEXT::INTEGER
    )
  );
...
CREATE POLICY reservas_insert_users ON reservas
  FOR INSERT WITH CHECK (
    auth.uid() IS NOT NULL AND usuario_id = auth.uid()::TEXT::INTEGER
  );
...
BEGIN
  LOOP
    ...
    SELECT EXISTS (
      SELECT 1 FROM reservas WHERE codigo_reserva = codigo
    ) INTO existe;
...
BEFORE INSERT ON reservas
FOR EACH ROW EXECUTE PROCEDURE generar_codigo_reserva();
